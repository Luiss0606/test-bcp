...
from app.services.analysis_service import FinancialAnalysisService, ReportGenerationService
...
async def analyze_customer_debt(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing customer debt: {str(e)}")
...
async def get_scenario_analysis(
    ...
    try:
        ...
        if scenario_type == "minimum":
            scenario = analysis_service.debt_calculator.calculate_minimum_payment_scenario(customer_id)
        elif scenario_type == "optimized":
            scenario = analysis_service.debt_calculator.calculate_optimized_scenario(customer_id)
        elif scenario_type == "consolidation":
            scenario = analysis_service.debt_calculator.calculate_consolidation_scenario(customer_id)
...
async def check_consolidation_eligibility(
    customer_id: str,
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        for offer in offers:
            customer_data = {
                ...
                "has_past_due": customer_info.get("has_past_due", False),
                "max_days_past_due": customer_info.get("max_days_past_due", 0)
            }
            
            if offer.is_eligible_for_customer(customer_data):
                # Calculate potential payment
                debts = analysis_service.debt_calculator.get_customer_debts(customer_id)
                consolidatable_balance = sum(
                    debt.balance for debt in debts 
                    if debt.debt_type in offer.product_types_eligible
                )
                
                if consolidatable_balance <= offer.max_consolidated_balance:
                    new_payment = offer.calculate_new_payment(consolidatable_balance)
                    
                    eligible_offers.append({
                        "offer_id": offer.id,
                        "new_rate_pct": offer.new_rate_pct,
                        "max_term_months": offer.max_term_months,
                        "consolidatable_balance": consolidatable_balance,
                        "estimated_payment": new_payment,
                        "conditions": offer.conditions
                    })
        ...
        return {
            ...
            "eligibility_count": len(eligible_offers)
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def get_analytics_summary(db: Session = Depends(get_db)):
    ...
    try:
        ...
        avg_loan_rate = sum(loan.annual_rate_pct for loan in loans) / len(loans) if loans else 0
        avg_card_rate = sum(card.annual_rate_pct for card in cards) / len(cards) if cards else 0
        
        customers_with_past_due = len(set(
        ...
        return {
            ...
            "past_due_rate": round(customers_with_past_due / total_customers * 100, 2) if total_customers > 0 else 0
        }
    except Exception as e:
